    ...
    if os.path.exists(profile_path):
        with open(profile_path, "r", encoding="utf-8") as file:
            if extension == ".py":
                ...
                # Remove `from open_interpreter.runtime import interpreter` and `interpreter = OpenInterpreter()`, because we handle that before the script
...
class RemoveInterpreter(ast.NodeTransformer):
    """Remove interpreter bootstrap statements from profile scripts."""

    def visit_ImportFrom(self, node):
        if node.module in {"interpreter", "open_interpreter.runtime"}:
